"""Proves Intermediate/Advanced continue the SAME SplitFarmingTargetEvent
Policy lineage Beginner produces -- no checkpoint bridge, no action-space
conversion, no fresh navigation policy, same frozen 0051200 -- matching
docs/architecture/CURRICULUM_TRAINING_PIPELINE.md section 4/6/8/9.
RUN_CANONICAL_INTERMEDIATE.py/RUN_CANONICAL_ADVANCED.py call exactly the
shared function exercised here (continue_farming_policy_ppo_chunk), only
varying `stage`/`canonical_stage` and the curriculum/manifest paths -- this
test proves that reuse actually works for the "intermediate"/"advanced"
...
from simulator.beginner_transition import continue_farming_policy_ppo_chunk
...
def test_continue_farming_policy_ppo_chunk_across_stage(tmp_path: Path, internal_stage: str, template: str) -> None:
    starting_model = build_fresh_basic_policy(seed=0, device="cpu")
    starting_checkpoint = tmp_path / f"{internal_stage}_start.zip"
    starting_model.save(str(starting_checkpoint))
    ...
    assert after.action_space == starting_model.action_space, (
        f"{internal_stage} continuation must preserve the exact action contract without any conversion"
    )
    ...
    # Same lineage, not a fresh navigation policy: the source's weights must
    # have moved from initialization (proving real training happened), and
    # the checkpoint's own architecture (net shapes) is unchanged from the
    # starting policy -- a genuine continuation, not a rebuild.
    before_params = dict(starting_model.policy.named_parameters())
