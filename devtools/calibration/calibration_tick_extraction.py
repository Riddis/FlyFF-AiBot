...
def main() -> None:
    default_csv = Path("simulator/run_logs/calibration_evidence/movement_calibration_steering.csv")
    csv_path = Path(sys.argv[1]) if len(sys.argv) > 1 else default_csv
    ticks = extract(csv_path)
    ticks.to_csv("simulator/run_logs/calibration_evidence/calibration_tick_extraction.csv", index=False)
