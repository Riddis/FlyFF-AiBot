...
|---|---|---|
| `test_navigation_dataset.py::test_mine_navigation_dataset_produces_all_four_categories_on_real_layouts` | `PRE_EXISTING_ENVIRONMENTAL/ARTIFACT` | **[Corrected 2026-08-18, see PHASE14_REPORT.md section 36 for the full forward-correction.]** The test calls `PPO.load("models/split_branch_pilot_15000.zip", device="cpu")` (`tests/test_navigation_dataset.py:107`). The primary defect is that `models/split_branch_pilot_15000.zip` is a historical checkpoint that predates consolidation and is not present in the preserved current worktree — it is not a tracked current model artifact. The observed `FileNotFoundError: models\split_branch_pilot_15000.zip.zip` is a *secondary* symptom: Stable-Baselines3's own missing-path/suffix-retry behavior when the given path does not exist, not proof that the primary defect is merely an extra `.zip` appended by the test. Pre-existing across every prior phase's baseline; not touched by migration. Not fixed this phase: the missing checkpoint was not fabricated, not substituted with an unrelated checkpoint (e.g. `0051200`) to force a pass, and not retrained — none of those would be evidence-backed. |
...
- `test_mine_navigation_dataset...` depends on the historical `models/split_branch_pilot_15000.zip` checkpoint, absent from the current worktree (corrected 2026-08-18; the `.zip.zip` in the error is a secondary SB3 symptom, not the primary defect) — pre-existing, unchanged.
- `apps/recorder_app.py` has no `--help`/argparse handling — newly documented. **Process lesson (corrected 2026-08-18):** this was discovered only after invoking `python -m apps.recorder_app --help` without first confirming via source inspection that `--help` terminated before any unsafe initialization; see `MISTAKES.md`.
