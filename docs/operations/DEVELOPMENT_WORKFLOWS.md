...
`requirements/base.txt` is the single base dependency set (bot/recorder
runtime only). Two layered extras each `-r base.txt` to add to
...
- `apps/dev_app.py`, `bot/recording_sink.py`, `runtime/recording_format.py`
