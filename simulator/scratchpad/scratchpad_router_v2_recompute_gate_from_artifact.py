...
def main() -> None:
    saved = json.loads((ROOT / "simulator" / "evaluations" / "router_v2_guarded_development_validation.json").read_text(encoding="utf-8"))
    ...
    a_640 = json.loads((ROOT / "simulator" / "evaluations" / "general_router_bridge_check_corrected_previous_steering.json").read_text(encoding="utf-8"))
    a_663 = json.loads((ROOT / "simulator" / "evaluations" / "beginner_routing_two_wall_s_route_eval_corrected_previous_steering.json").read_text(encoding="utf-8"))
    a_fixtures = json.loads((ROOT / "simulator" / "evaluations" / "routing_regression_fixtures_result_pre_selector_patch.json").read_text(encoding="utf-8"))
    ...
    saved["gate"] = gate
    out_path = ROOT / "simulator" / "evaluations" / "router_v2_guarded_development_validation.json"
