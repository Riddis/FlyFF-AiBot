...
def main() -> None:
    verify_historical_snapshot(
        extra_files=(f"simulator/evaluations/router_mix_final_pool_{FINAL_CONFIRMATION_SPEC_SEED}_manifest.json",),
    )
    manifest = load_manifest(ROOT / "simulator" / "evaluations" / f"router_mix_final_pool_{FINAL_CONFIRMATION_SPEC_SEED}_manifest.json")
    ...
    out_path = ROOT / "simulator" / "evaluations" / f"router_v2_final_confirmation_{FINAL_CONFIRMATION_SPEC_SEED}_result.json"
