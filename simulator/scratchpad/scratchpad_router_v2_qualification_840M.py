...
def main() -> None:
    verify_historical_snapshot(
        extra_files=(f"simulator/evaluations/router_mix_qualification_pool_{QUALIFICATION_SPEC_SEED}_manifest.json",),
    )
    ...
    checksums = json.loads((ROOT / "simulator" / "evaluations" / f"router_mix_qualification_pool_{QUALIFICATION_SPEC_SEED}_checksums.json").read_text(encoding="utf-8"))
    ...
    out_path = ROOT / "simulator" / "evaluations" / f"router_v2_qualification_{QUALIFICATION_SPEC_SEED}_result.json"
