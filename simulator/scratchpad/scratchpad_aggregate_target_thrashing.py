...
def main() -> None:
    ...
    for shard_idx in range(8):
        path = ROOT / "simulator" / "evaluations" / f"target_thrashing_measurement_shard{shard_idx}.json"
        all_episodes.extend(json.loads(path.read_text(encoding="utf-8")))
    missing_path = ROOT / "simulator" / "evaluations" / "target_thrashing_measurement_shard_missing.json"
    ...
    out_path = ROOT / "simulator" / "evaluations" / "target_thrashing_aggregate_report.json"
