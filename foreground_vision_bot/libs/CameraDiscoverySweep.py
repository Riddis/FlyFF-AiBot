...
class CameraDiscoveryConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.focus_timeout_seconds <= 0.0:
            raise ValueError("focus_timeout_seconds must be positive")
        if self.focus_poll_seconds <= 0.0:
            raise ValueError("focus_poll_seconds must be positive")
        if self.focus_settle_seconds < 0.0:
...
class CameraDiscoverySweep:
    ...
    def _wait_for_target_focus(self, keyboard) -> bool:
        ...
        timeout = float(self.config.focus_timeout_seconds)
        focus_target = getattr(keyboard, "focus_target_window", None)
        if self.config.auto_focus_on_start and callable(focus_target):
            self._status(
                "FlyFF is not focused; attempting to activate the game window "
                "before camera discovery."
            )
            try:
                focus_target()
            except Exception:
                # Automatic focus is best-effort. The manual grace period below
                # remains the reliable fallback on restrictive Windows setups.
                pass

        if is_foreground():
            self._status(
                "FlyFF window focused automatically; camera discovery begins shortly."
            )
            self._wait(self.config.focus_settle_seconds)
            return not self.cancellation.cancelled and self.bot.rl_enabled

        self._status(
            f"Click the FlyFF window now. Camera discovery will wait up to "
            f"{timeout:.0f} seconds for focus."
        )
        deadline = monotonic() + timeout
        next_retry = monotonic() + 1.0
        while monotonic() < deadline:
            ...
            now = monotonic()
            if (
                self.config.auto_focus_on_start
                and callable(focus_target)
                and now >= next_retry
            ):
                try:
                    focus_target()
                    ...
                    pass
                next_retry = now + 1.0
            self._wait(min(self.config.focus_poll_seconds, max(0.0, deadline - now)))

        raise RuntimeError(
            "FlyFF did not gain focus before camera discovery. Click the game "
            "window and start the run again, or increase focus_timeout_seconds."
        )
