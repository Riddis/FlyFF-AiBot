...
def _patch_native_farming_env() -> None:
    ...
    def patched_init(self, *args: Any, **kwargs: Any) -> None:
        ...
        _ensure_map_context(self, required=True)
...
def _reset_unified_state(env: NativeFarmingEnv) -> None:
    ...
    env._v0700_map_source = "--"
    env._v0700_map_path = "--"
    env._v0700_map_shape = None
    env._v0700_map_blocked_cells = 0
    env._v0700_player_map_cell = None
...
def _build_unified_observation(env: NativeFarmingEnv) -> np.ndarray:
    ...
    pose = _read_pose(env)
    context = _ensure_map_context(env, required=True)
    map_adapter = _MapAdapter(context)
    ...
    env._v0700_map_path = map_adapter.map_path
    env._v0700_map_shape = map_adapter.shape
    env._v0700_map_blocked_cells = map_adapter.blocked_cell_count
    env._v0700_player_map_cell = player_cell
...
def _build_info(
    ...
    *,
    ...
    info: dict[str, object] = {
        ...
        "map_source": str(getattr(env, "_v0700_map_source", "--")),
        "map_path": str(getattr(env, "_v0700_map_path", "--")),
        "map_shape": getattr(env, "_v0700_map_shape", None),
        "map_blocked_cells": int(getattr(env, "_v0700_map_blocked_cells", 0)),
        "player_map_cell": getattr(env, "_v0700_player_map_cell", None),
...
def _ensure_map_context(env: NativeFarmingEnv, *, required: bool) -> Any | None:
    """Resolve the already-loaded map context used by the live environment.

    v0.7.0 originally looked only for generic ``grid`` attributes.  This
    project passes a ``NativeMapContext`` whose occupancy lives in
    ``safe_traversable`` and ``layout.traversable``.  Resolve that concrete
    object first, while retaining fallbacks for partially upgraded runtimes.
    """
    candidates: tuple[tuple[str, Any], ...] = (
        ("env.map_context", getattr(env, "map_context", None)),
        (
            "env.observation_builder.map_context",
            getattr(getattr(env, "observation_builder", None), "map_context", None),
        ),
        (
            "env.navigator.map_context",
            getattr(getattr(env, "navigator", None), "map_context", None),
        ),
        (
            "env.navigator.controller",
            getattr(getattr(env, "navigator", None), "controller", None),
        ),
        (
            "env.navigator.executor",
            getattr(getattr(env, "navigator", None), "executor", None),
        ),
        ("env.bot.map_context", getattr(getattr(env, "bot", None), "map_context", None)),
    )
    for source, candidate in candidates:
        if candidate is None:
            continue
        nested = getattr(candidate, "map_context", None)
        contexts = (candidate, nested) if nested is not None else (candidate,)
        for context in contexts:
            adapter = _MapAdapter(context)
            if not adapter.available:
                continue
            env.map_context = context
            env._v0700_map_source = source
            return context

    if required:
        raise RuntimeError(
            "Unified farming requires the mapped occupancy context. Select a "
            "completed coordinate map and restart RL control before training."
        )
    return None


class _MapAdapter:
    """Adapter for the concrete NativeMapContext plus older map revisions."""

    def __init__(self, context: Any) -> None:
        self.context = context
        self.safe_traversable = self._find_safe_traversable(context)
        self.traversable = self._find_traversable(context)
        self.forbidden = self._find_forbidden(context)
        ...
        self.available = context is not None and (
            self.safe_traversable is not None
            or self.traversable is not None
            or self.grid is not None
            or self._has_cell_query(context)
        )

    @property
    def shape(self) -> tuple[int, int] | None:
        for array in (self.safe_traversable, self.traversable, self.grid):
    ...
    def map_path(self) -> str:
        directory = getattr(self.context, "map_directory", None)
        return "--" if directory is None else str(directory)

    @property
    def blocked_cell_count(self) -> int:
        if self.safe_traversable is not None:
            return int(self.safe_traversable.size - np.count_nonzero(self.safe_traversable))
        if self.traversable is not None:
            return int(self.traversable.size - np.count_nonzero(self.traversable))
        if self.grid is None:
            return 0
        states = np.fromiter(
            (self._decode_cell_value(value) for value in self.grid.flat),
            dtype=np.float32,
            count=int(self.grid.size),
        )
        return int(np.count_nonzero(states > 0.5))

    @staticmethod
    def _find_safe_traversable(context: Any) -> np.ndarray | None:
        value = getattr(context, "safe_traversable", None)
        if isinstance(value, np.ndarray) and value.ndim == 2:
            return np.asarray(value, dtype=np.bool_)
        return None

    @staticmethod
    def _find_traversable(context: Any) -> np.ndarray | None:
        layout = getattr(context, "layout", None)
        value = getattr(layout, "traversable", None)
        if isinstance(value, np.ndarray) and value.ndim == 2:
            return np.asarray(value, dtype=np.bool_)
        value = getattr(context, "traversable", None)
        if isinstance(value, np.ndarray) and value.ndim == 2:
            return np.asarray(value, dtype=np.bool_)
        return None

    @staticmethod
    def _find_forbidden(context: Any) -> np.ndarray | None:
        layout = getattr(context, "layout", None)
        value = getattr(layout, "forbidden", None)
        if isinstance(value, np.ndarray) and value.ndim == 2:
            return np.asarray(value, dtype=np.bool_)
        value = getattr(context, "forbidden", None)
        if isinstance(value, np.ndarray) and value.ndim == 2:
            return np.asarray(value, dtype=np.bool_)
        return None
    ...
    def normalized_position(
        ...
        shape = self.shape
        if cell is not None and shape is not None:
            height, width = shape
    ...
    def cell_state(self, cell: tuple[int, int]) -> float:
        ...
        if shape is None:
            return 0.0
        if z < 0 or x < 0 or z >= shape[0] or x >= shape[1]:
            return 0.0
        if self.safe_traversable is not None:
            return -1.0 if bool(self.safe_traversable[z, x]) else 1.0
        if self.traversable is not None:
            if bool(self.traversable[z, x]):
                return -1.0
            if self.forbidden is not None and bool(self.forbidden[z, x]):
                return 1.0
            return 1.0
        if self.grid is None:
            return 0.0
