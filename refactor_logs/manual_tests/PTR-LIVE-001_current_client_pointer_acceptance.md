...
8. Expected second result is recovery success using
   ...
   `layout.world_vtable_offset`, plus `layout.world_identity_kind`. Adjacent
   pre-recovery backups must preserve the prior files.
9. Click **Native Health** once. Expect `healthy`, the same module identity,
   ...
   consistent with the short movement. It must also report the persisted
   `world_identity_kind`. Any world-identity error or missing persisted
   identity field/table-or-marker is a stop condition; do not run dry-run.
