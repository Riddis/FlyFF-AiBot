...
class AdaptiveMapper:
    ...
    def __init__(
        ...
        self._blocked_observations: dict[tuple[int, int, int, int], int] = {}
    ...
    def _execute_forward(self, step: int) -> _StepResult:
        ...
        if assessment.outcome is AdaptiveForwardOutcome.BLOCKED:
            ...
            stop_reason = self._record_blocked_observation(
                from_cell=(self.grid.pose.x, self.grid.pose.y),
                target_cell=target_cell,
                heading_deg=strict_heading.angle_deg,
            )
            return _StepResult(
                frame_sample=after,
                fast_heading=fast_heading,
        ...
        self._blocked_observations.pop(
            self._contact_boundary_key(
                (self.grid.pose.x, self.grid.pose.y),
                target_cell,
            ),
            None,
        )
    ...
    @staticmethod
    def _contact_boundary_key(
        from_cell: tuple[int, int],
        target_cell: tuple[int, int],
    ) -> tuple[int, int, int, int]:
        first, second = sorted((tuple(from_cell), tuple(target_cell)))
    ...
    def _record_blocked_observation(
        ...
        *,
        ...
        boundary_key = self._contact_boundary_key(from_cell, target_cell)
        observations = self._blocked_observations.get(boundary_key, 0) + 1
        self._blocked_observations[boundary_key] = observations
        required = max(1, self.config.blocked_confirmations)
        if observations < required:
            self.status_callback(
                "Potential obstacle boundary needs confirmation "
                f"({observations}/{required}); map unchanged."
            )
            return None

        self._blocked_observations.pop(boundary_key, None)
        ...
        if blocked_recorded:
            self.status_callback(
                "Obstacle boundary confirmed from "
                f"({from_cell[0]}, {from_cell[1]}) to "
                f"({target_cell[0]}, {target_cell[1]}); target cell marked "
                "blocked and replanning around it."
            )
        elif target_value == FREE:
            self.status_callback(
                "Obstacle boundary confirmed from "
                f"({from_cell[0]}, {from_cell[1]}) to "
                f"({target_cell[0]}, {target_cell[1]}). The target cell was "
                "previously observed free from another direction, so the free "
                "cell is preserved while this failed crossing is blocked."
            )
        else:
            self.status_callback(
                "Obstacle boundary confirmed from "
                f"({from_cell[0]}, {from_cell[1]}) to "
                f"({target_cell[0]}, {target_cell[1]}); replanning around the "
                "existing blocked boundary."
            )

        # Contact is edge evidence. It is never a reason to discard an otherwise
        # trusted pose merely because coarse odometry rounded the target into a
        # cell that was visited from another side.
        return None
