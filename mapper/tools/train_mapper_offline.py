...
# Two directories deep under the repository root (mapper/tools/); direct
# invocation sets sys.path[0] to this file's own directory, not the
# repository root, so the mapper package below needs it added explicitly
# (same bootstrap pattern devtools/native/inspect_native_monsters.py uses).
APP_ROOT = Path(__file__).resolve().parents[2]
if str(APP_ROOT) not in sys.path:
    sys.path.insert(0, str(APP_ROOT))

from mapper.rl.OfflineTraining import (  # noqa: E402
    config_to_dict,
