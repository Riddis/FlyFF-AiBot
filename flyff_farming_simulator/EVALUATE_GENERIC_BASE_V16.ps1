...
$output = "evaluations\generic_farming_v16_${stageTag}_${mode}.json"

$args = @(
    "-u", "run_fair_time_simulator.py", "evaluate-synthetic",
...
Write-Host "Fair-time evaluation: $EpisodesPerLayout episode(s)/layout, $EpisodeSeconds simulated seconds, $MaxActions action cap."
& $python @args
$exitCode = $LASTEXITCODE
if ($exitCode -eq 3) {
    throw "The checkpoint failed the synthetic stage gate. Results: $output"
}
if ($exitCode -ne 0) { throw "Fair-time generic-base evaluation failed with exit code $exitCode." }
