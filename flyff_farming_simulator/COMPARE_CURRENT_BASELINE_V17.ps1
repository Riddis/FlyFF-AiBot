...
$output = "evaluations\current_baseline_reward_audited_${mode}.json"

Write-Host "Matched reward-audited comparison: $Episodes episode(s), $EpisodeSeconds simulated seconds."
& $python -u run_reward_audited_simulator.py compare-policies `
    $WorldModel `
    ...
    --output $output
if ($LASTEXITCODE -ne 0) { throw "Reward-audited policy comparison failed." }
