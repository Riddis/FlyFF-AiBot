...
function Train-Stage {
    ...
    $args = @(
        "-u", "run_reward_audited_simulator.py", "train-synthetic",
    ...
    & $python @args
    if ($LASTEXITCODE -ne 0) { throw "$Stage reward-audited training failed." }
}
...
function Test-StageGate {
    ...
    & $python -u run_reward_audited_simulator.py evaluate-synthetic `
        synthetic_curriculum\curriculum.json `
        $Checkpoint `
        --stage $Stage `
        ...
        --torch-threads 1 `
        --progress-every 1 `
        --output "evaluations\generic_${Stage}_stage_gate_v17.json"
    if ($LASTEXITCODE -eq 3) {
        throw "$Stage checkpoint failed its reward-audited stage gate. Training stopped before the next stage."
    }
