"""2026-08-14: shared infrastructure for evaluating the qualified
waypoint navigator under the GENERAL `kinodynamic_route_planner`
(plan_route + select_persistent_waypoint), per explicit user instruction
-- introduced as a genuinely new routing-layer experiment (not a silent
extension of the frozen single-wall `compute_subgoal_cells` formula,
which stays unchanged as the regression reference for that geometry).

Used by both:
  - scratchpad_general_router_bridge_check.py (compatibility pass on the
    EXISTING step-1 randomized-wall pool, no tuning)
  - scratchpad_beginner_routing_two_wall_s_route.py (fresh two-wall
    S-route suite)

Control-loop design (standard for this planner, per its own docstrings):
plan_route() is called ONCE per episode from the start state to the true
final destination. select_persistent_waypoint() is then called EVERY
tick against that same fixed route, given the player's current position/
heading -- it is not a one-shot subgoal committed to until reached (the
single-wall formula's design); it continuously re-selects the best
persistent target to chase along the already-planned route. No planner
constants are changed anywhere in this module.

Because the target can legitimately change value tick-to-tick (not just
at one fixed handoff moment), "number of waypoints issued" here means
DISTINCT persistent-target values over the episode (deduped by a small
position tolerance), not a fixed enumerated list -- documented explicitly
since it's a real definitional choice, not hidden. "Route progress
fraction" (nearest route-node index / route length) gives a continuous,
route-native failure-location measure in place of a discrete subgoal
index, used to classify failures as early/mid/late along the route
rather than a fixed number of named stages.
"""
...
from simulator.static_waypoint_env import FIXED_HEADING, SUCCESS_RADIUS_CELLS, SYMMETRIC_MOVEMENT
