...
class RecoveredNativeProfile:
    ...
    def from_dict(cls, payload: Mapping[str, object]) -> "RecoveredNativeProfile":
        ...
        if isinstance(raw_presence_evidence, Mapping):
            evidence_fields = {
                ...
                for field in (
                    "offset", "selected_matches", "selected_samples",
                    "zero_hp_matches", "zero_hp_samples", "dormant_clears",
                    "dormant_samples", "cross_slot_alias_matches",
                    "lifecycle_death_retained", "lifecycle_dormant_clears",
        ...
        profile = cls(
            ...
            presence_species_offset=(None if payload.get("presence_species_offset") is None else int(payload["presence_species_offset"])),
            presence_species_validated=bool(payload.get("presence_species_validated", False)),
    ...
    def validate(self) -> None:
        ...
        if self.presence_evidence is not None and self.presence_species_offset != self.presence_evidence.offset:
...
def profile_from_reader(
    *,
    ...
    profile = RecoveredNativeProfile(
        ...
        presence_species_offset=(None if presence_species_offset is None else int(presence_species_offset)),
...
def _restore_runtime_cache(
    ...
    *,
    ...
    presence_candidates = _presence_field_candidates(
        ...
        preferred_offsets=tuple(dict.fromkeys(value for value in (profile.presence_species_offset, 0x1DCC) if value is not None)),
    )
...
def restore_profile(
    ...
    *,
    ...
    authoritative = refresh_authoritative_actors(
        ...
        preferred_presence_offsets=tuple(dict.fromkeys(value for value in (profile.presence_species_offset, 0x1DCC) if value is not None)),
    )
