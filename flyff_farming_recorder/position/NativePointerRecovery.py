...
def _verify_cached(
    ...
    try:
        if cached.independent_discovery is not None:
            ...
            if player <= 0 or player != int(target.base):
                # The module slot may still resolve to a valid player-shaped
                # object after a relog, teleport, or player-object rebuild, but
                # the cached independent discovery embeds the previous heap
                # base and actor anchors. Reusing that stale discovery lets the
                # cheap verifier succeed and then makes IndependentNativeReader
                # fail when it validates the cached direct aliases. Invalidate
                # the cache here so the persisted-profile or full-recovery path
                # can rebuild coherent current-process metadata.
                return False
            if not any(
