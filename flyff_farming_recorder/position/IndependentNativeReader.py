...
class IndependentNativeReader:
    ...
    standalone recorder may explicitly enable a separately validated presence
    ...
    def __init__(
        ...
        *,
        ...
        if (
            ...
            self._recovered_presence_species_offset = restored_authoritative.presence_species_offset
            self._presence_species_validated = bool(restored_authoritative.presence_species_validated)
            self._presence_validation_source = (
                "restored_build_profile"
                if self._presence_species_validated
                else "unproven"
            )
            self._authoritative_presence_candidates = tuple(restored_authoritative.presence_candidates)
            ...
            try:
                ...
                if authoritative.succeeded:
                    ...
                    self._recovered_presence_species_offset = authoritative.presence_species_offset
                    self._presence_species_validated = bool(authoritative.presence_species_validated)
                    self._presence_validation_source = (
                        "authoritative_initial_discovery"
                        if self._presence_species_validated
                        else "unproven"
                    )
                    self._authoritative_presence_candidates = tuple(authoritative.presence_candidates)
                    self._last_authoritative_refresh_at = monotonic()
                    if status_callback is not None:
                        scan_elapsed = max(
        ...
        # Recorder-only optimized presence sampling.  A dynamically observed
    ...
    def presence_validation_source(self) -> str:
        return str(getattr(self, "_presence_validation_source", "unproven"))
    ...
    def promote_validated_presence_offset(
        ...
        *,
        ...
        """Install a longitudinally proven presence field during a recording.

        The profiler supplies dormant-transition evidence. The reader still
        performs an independent current-process sanity check before the field can
        influence hot/cold polling. This separates semantic discovery from the
        optimized sampler and prevents a coincidental duplicate-species field
        from being promoted blindly.
        """
        ...
        species_count = len(species_values) if isinstance(species_values, (list, tuple, set)) else 0
        ...
        for base in actor_slots[:256]:
            try:
                species = _i32(
                    self._memory.read(int(base) + int(layout.species_offset), 4)
                )
                if species not in selected:
                    continue
                value = _i32(self._memory.read(int(base) + resolved, 4))
            except Exception:
                continue
            current_samples += 1
        ...
        candidate = PresenceFieldCandidate(
            offset=resolved,
            selected_matches=live_matches,
            selected_samples=live_samples,
            zero_hp_matches=int(evidence.get("zero_hp_matches", 0)),
            zero_hp_samples=int(evidence.get("zero_hp_samples", 0)),
            dormant_clears=max(0, dormant_samples - dormant_matches),
            dormant_samples=dormant_samples,
            cross_slot_alias_matches=0,
            lifecycle_death_retained=int(evidence.get("zero_hp_matches", 0)),
            lifecycle_dormant_clears=max(0, dormant_samples - dormant_matches),
            lifecycle_reappearances=int(evidence.get("reappearance_matches", 0)),
            validated=True,
        )
        others = tuple(
            item for item in self._authoritative_presence_candidates
        ...
        self._presence_validation_source = str(source or "session_longitudinal_profiler")
    ...
    def enable_presence_optimized_sampling(
        ...
        *,
        ...
        """Enable accuracy-first hot/cold polling for the standalone recorder.
        ...
        try:
            ...
            with self._cache_lock:
                ...
                if refreshed.presence_species_validated:
                    self._recovered_presence_species_offset = refreshed.presence_species_offset
                    self._presence_species_validated = True
    ...
    def _observe_presence_candidates(self, base: int, species: int, hp: int) -> None:
        ...
        for candidate in self._authoritative_presence_candidates[:8]:
            ...
            support = self._presence_runtime_support.setdefault(
                offset,
                {
                    "selected_matches": 0, "selected_samples": 0,
                    "zero_matches": 0, "zero_samples": 0,
                    "dormant_clears": 0, "dormant_samples": 0,
                    "death_retained": 0, "dormant_transitions": 0,
                    "reappearances": 0,
                },
            )
            ...
            if species in selected and hp >= 0:
                ...
                if previous is not None and previous[0] == species and previous[1] > 0 and hp == 0 and value == species:
                    support["death_retained"] += 1
                if previous is not None and previous[0] <= 0 and previous[2] == 0 and value == species:
                    support["reappearances"] += 1
            elif species <= 0:
                support["dormant_samples"] += 1
                support["dormant_clears"] += int(value == 0)
                if previous is not None and previous[0] > 0 and previous[2] == previous[0] and value == 0:
                    support["dormant_transitions"] += 1
            self._presence_runtime_previous[(offset, int(base))] = (int(species), int(hp), int(value))
            ...
            validated = bool(
                ...
                and support["dormant_transitions"] >= 1
